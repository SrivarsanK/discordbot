...
module.exports = {
  ...
  cooldown: 5,
  description: "View the LeetCode leaderboard (overall/weekly/monthly/questions)",
  args: false,
  usage: "",
  userPerms: [],
  botPerms: [],
  ...
  execute: async (message, args, client) => {
    ...
    const memberCache = relevantUserIds.length > 0 
      ? await message.guild.members.fetch({ user: relevantUserIds }).catch(() => null) 
      : null;
    ...
    collector.on("collect", async (interaction) => {
      ...
      if (interaction.isStringSelectMenu() && interaction.customId === "leaderboard_select") {
        currentSelectedType = interaction.values[0];
        currentPage = 1;
